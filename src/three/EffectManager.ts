...
export class EffectManager {
  ...
  renderAsciiPass(
    ...
    // 只有在 About 模式下才需要动态追踪模型中心（因为 About 页面人物偏离中心很远）
    // Home 模式下我们投影世界原点 (0,0,0)，这样人物绕原点旋转的动态能被保留
    if (options.mode === 'about' && model) {
    ...
    for (let r = this.asciiRows - 1; r >= 0; r--) {
      ...
      // 3D 轮廓边界检测 (仅 Home 模式使用简单的左侧截断逻辑)
      ...
      for (let c = 0; c < this.asciiCols; c++) {
        ...
        if (options.mode === 'about' && mx >= 0 && mx < this.maskWidth && my >= 0 && my < this.maskHeight) {
  ...
  renderAboutAsciiPass(
    ...
    // 我们希望 pxX * zoom (放大的投影点) 处于 subWidth (视口窗口) 的中心
    // 增加一个垂直偏移量 (15% 屏幕高度) 以将人物图像向上移动
    ...
    for (let r = this.aboutAsciiRows - 1; r >= 0; r--) {
      ...
      for (let c = 0; c < this.aboutAsciiCols; c++) {
        ...
        // 1. 规避 3D 轮廓 (基于全屏 maskBuffer，并增加邻域检测实现 Padding)
        // 使用字符中心采样，提高避障精准度
        ...
        let hitModel = false
        const searchRadius = 2 // 5x5 邻域，为人物提供约 1 字符宽的保护边距
